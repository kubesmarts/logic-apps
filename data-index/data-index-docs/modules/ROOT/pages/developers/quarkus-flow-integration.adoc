...
The MODE 1 PostgreSQL triggers parse the event's `timestamp`, `startTime`, and `endTime`
fields with `to_timestamp(... ::numeric)`, so they must be Unix epoch seconds.
====
...
# CRITICAL: namespace must be 'workflows' for the default Vector config
...
Quarkus Flow automatically creates a file handler for structured logging. In Kubernetes, pods don't have write access to `/var/log/quarkus-flow/`, causing startup errors. Disabling the file handler ensures logs only go to stdout, which Vector can capture.
====
...
|Wrong namespace
|Must be `workflows` (or update the Vector `WORKFLOW_NAMESPACE`)
