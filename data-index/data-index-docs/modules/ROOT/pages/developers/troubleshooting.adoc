...
Common issues and solutions when integrating Quarkus Flow apps with Data Index.
...
**Cause:** Using Maven profile but not activating it.
...
Or if using `-Dquarkus.profile=kubernetes`, ensure your Maven profile is activated by this property.
...
**Cause:** Missing `quarkus-kind` extension when using `deployment-target=kind`.

**Solution:**

Add to your Maven profile:
...
2. Verify Flyway migrations completed successfully:
+
[source,bash]
----
kubectl exec -n postgresql postgresql-0 -- \
