...
2. **Kubernetes deployment** - Run in Kubernetes where Vector can collect logs
...
The MODE 1 PostgreSQL triggers parse the event's `timestamp`/`startTime`/`endTime` fields
with `to_timestamp(... ::numeric)`, so they must be Unix epoch seconds. Using `epoch-seconds`
ensures proper timestamp parsing.
