...
[IMPORTANT]
====
Deployment is now **Helm-based**. The fastest path is the one-command Makefile
targets, which create the cluster, build images, install the chart, and run the
tests:

[source,bash]
----
make m1   # PostgreSQL + Vector + triggers
make m2   # Elasticsearch + Vector + transforms
make m3   # Kafka ingestion
----
...
[source,bash]
----
kind load docker-image kubesmarts/data-index-service:2.0.0-SNAPSHOT --name data-index-test
...
[source,bash]
----
# Verify the namespace filter Vector's kubernetes_logs source applies
kubectl get configmap -n logging vector-config -o yaml | grep -A1 extra_field_selector
----
...
[source,bash]
----
docker rmi kubesmarts/data-index-service:2.0.0-SNAPSHOT
