= FluentBit Configuration (DEPRECATED)
:page-aliases:
...
Both MODE 1 (PostgreSQL) and MODE 2 (Elasticsearch) now use Vector as the log
collector. See xref:deployment/vector-config.adoc[Vector Configuration].

This page is retained for one release to support in-place migration. The FluentBit
Helm templates ship disabled (`fluentbit.enabled: false`); opt back in with
