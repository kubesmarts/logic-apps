...
TIP: The Lua script is embedded in the ConfigMap. After modifying `flatten-event.lua`, regenerate the ConfigMap using `generate-configmap.sh` and restart FluentBit pods.
