...
2. **Vector** collects container logs and forwards events (xref:deployment/vector-config.adoc[Vector Configuration])
...
[source,text]
----
Quarkus Flow → Vector → PostgreSQL (raw tables)
                                ↓ (triggers)
...
[source,text]
----
Quarkus Flow → Vector → Elasticsearch (raw indices)
                                ↓ (ES Transform)
...
* Accessible from Data Index service and Vector

**Vector:**

* Vector 0.46+ (native `postgres` sink)
