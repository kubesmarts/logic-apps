...
[source,text]
----
Quarkus Flow Apps → Container Logs → Vector DaemonSet (postgres sink)
                                           ↓
...
Vector collects workflow events from container logs and writes them to the
PostgreSQL raw tables via its native `postgres` sink. It is deployed by the
Data Index Helm chart (enabled by default for MODE 1) — no separate step is
needed if you installed the chart in Step 3 with `values-mode1.yaml`.

Config source of truth: `data-index/collectors/vector/mode1-postgresql/vector.yaml`
(see xref:deployment/vector-config.adoc[Vector Configuration]).
...
vector:
  enabled: true
  config: mode1-postgresql      # selects configs/vector/vector-mode1-postgresql.yaml
  image:
    tag: "0.54.0-distroless-libc"
  workflowNamespace: workflows  # namespace whose container logs are tailed
  debugEvents: "false"          # set "true" to trace events to stdout
----

The chart renders the Vector ConfigMap and DaemonSet into the `logging`
namespace and passes `POSTGRES_HOST`/`POSTGRES_PORT`/`POSTGRES_DB`/`POSTGRES_USER`/`POSTGRES_PASSWORD`
from the `postgresql.*` values (matching the FluentBit DaemonSet it replaces).
For a managed database, override `postgresql.*` accordingly.
...
# Confirm the postgres sinks are healthy (no repeated connection errors)
kubectl logs -n logging -l app=vector | grep -iE "postgres|error"
----
...
[IMPORTANT]
====
Vector is configured to tail logs from the `workflows` namespace by default.

Deploy your applications to this namespace or set `vector.workflowNamespace`
(DaemonSet env `WORKFLOW_NAMESPACE`).
====
...
metadata:
  ...
  namespace: workflows  # Important: must match Vector's WORKFLOW_NAMESPACE
spec:
  template:
    spec:
      containers:
...
**2. Check Vector collected events:**

[source,bash]
----
kubectl logs -n logging -l app=vector | grep -i "workflow"

# Or trace events explicitly:
...
* The `postgres` sinks use the default in-memory buffer with a 1s batch flush
  (matches the FluentBit MODE 1 setup). The `kubernetes_logs` source checkpoints,
  so a Vector restart re-reads recent lines and the idempotent triggers dedupe.
...
Tune sink batching for high throughput in
...
sinks:
  postgres_workflow:
    batch:
      max_events: 500      # larger batches = fewer INSERTs
      timeout_secs: 1
----

NOTE: Keep the default in-memory buffer. In Vector 0.54 the `postgres` sink does
not drain a small `buffer.type: disk` until graceful shutdown, which strands
low-volume events.
...
* Vector `vector_component_received_events_total` / `vector_component_sent_events_total`
...
* xref:deployment/vector-config.adoc[Vector Configuration]
