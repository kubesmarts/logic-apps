...
3. **Container images built**
   - Data Index service: `kubesmarts/data-index-service-elasticsearch:2.0.0-SNAPSHOT`
   - Vector: `timberio/vector:0.41.1-distroless-libc` (version managed in `data-index/collectors/pom.xml`)
...
kind load docker-image \
  kubesmarts/data-index-service-elasticsearch:2.0.0-SNAPSHOT \
...
spec:
  ...
  template:
    ...
    spec:
      containers:
      - name: data-index-service
        image: kubesmarts/data-index-service-elasticsearch:2.0.0-SNAPSHOT
...
* **RBAC**: Limit Vector permissions to necessary APIs (namespaces, pods, nodes: get/list/watch)

=== High Availability

* **Elasticsearch cluster**: 3+ nodes (multi-AZ deployment)
* **Data Index replicas**: Run 2+ instances behind load balancer
* **Vector**: DaemonSet runs on every node automatically
