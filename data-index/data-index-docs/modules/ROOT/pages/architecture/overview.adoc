...
Vector runs on each Kubernetes node and:

* Tails container logs for pods in the `workflows` namespace (`kubernetes_logs` source)
* Keeps only JSON events whose `eventType` starts with `io.serverlessworkflow.`
* Routes workflow vs task events (VRL `route` transform)
* Forwards to the storage backend (`postgres` sink for MODE 1, `elasticsearch` sink for MODE 2)

**Configuration:**

* `data-index/collectors/vector/mode1-postgresql/vector.yaml` (MODE 1)
* `data-index/collectors/vector/mode2-elasticsearch/vector.yaml` (MODE 2)
* Deployed via ConfigMap + DaemonSet by the Helm chart

Vector replaced FluentBit per ADR-0001 (OpenShift alignment). See
...
4. **Vector** tails container logs, extracts JSON events
...
**Note:** End-to-end latency includes Vector log tailing and batching, network transit, and storage processing. The primary difference is normalization speed: PostgreSQL triggers are synchronous (< 1ms), while Elasticsearch transforms run asynchronously (~1s).
...
[source,text]
----
Vector (postgres sink) → PostgreSQL raw tables (workflow_events_raw, task_events_raw)
...
[source,text]
----
Vector (elasticsearch sink) → Elasticsearch raw indices (workflow-events-*, task-events-*)
