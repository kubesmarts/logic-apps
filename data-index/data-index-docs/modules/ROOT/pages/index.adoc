...
Data Index is a *read-only query service* for Open Workflow 1.0.0 runtime execution data. It provides a GraphQL API for querying workflow instances and task executions from Quarkus Flow applications.
...
* 📝 **Structured Logging** - Captures events from Quarkus Flow apps via Vector
...
* Vector 0.46+ (deployed by the Helm chart)
