...
NOTE: Development mode doesn't include the Vector log collector.
To test with real workflow events, use the KIND installation below.
...
# 3. Vector (the log collector) is deployed by the Helm chart for MODE 1.
#    Full one-command flow: bash data-index/scripts/e2e/full-test-mode1.sh
----
...
# 3. Vector (the log collector) is deployed by the Helm chart for MODE 2.
#    Full one-command flow: bash data-index/scripts/e2e/full-test-mode2.sh
----
...
logging       vector-xxx                             1/1     Running
